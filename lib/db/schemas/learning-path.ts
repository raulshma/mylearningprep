...
// Learning objective within a topic
export const LearningObjectiveSchema = z.object({
  id: z.string().min(1),
  description: z.string().min(1),
  isCore: z.boolean().default(true), // Core objectives are required, non-core are supplementary
});
...
// Subtopic for breaking down complex topics
export const SubtopicSchema = z.object({
  id: z.string().min(1),
  title: z.string().min(1),
  description: z.string().min(1),
  estimatedMinutes: z.number().int().min(5).default(15),
});
...
export const LearningTopicSchema = z.object({
  ...
  // Extended fields for more comprehensive learning paths
  learningObjectives: z.array(LearningObjectiveSchema).default([]),
  subtopics: z.array(SubtopicSchema).default([]),
  keyConceptsToMaster: z.array(z.string()).default([]),
  commonMistakes: z.array(z.string()).default([]),
  realWorldApplications: z.array(z.string()).default([]),
  estimatedMinutes: z.number().int().min(10).default(30),
  resources: z.array(TopicResourceSchema).default([]),
  interviewRelevance: z.string().optional(), // Why this topic matters for interviews
  order: z.number().int().min(0).default(0), // Suggested order in the learning path
});
...
export type TopicResource = z.infer<typeof TopicResourceSchema>;
