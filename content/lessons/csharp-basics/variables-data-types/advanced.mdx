...
- `Span<T>` is a ref struct - lives only on stack, points to existing memory
- Slice creates a view, not a copy - modifications affect original!
- `stackalloc` allocates on stack instead of heap - great for temp buffers
...
- **Value Parameter**: Copies all bytes on every call - expensive for large structs!
- **ref Parameter**: Passes pointer (8 bytes) - caller sees modifications
- **in Parameter**: Readonly reference - best for large readonly structs
- **out Parameter**: Must be assigned before method returns - used for multi-value returns
...
- **Record Class**: Reference type with value-based equality and immutability
- **Record Struct**: Value type with generated equality - great for small data
- **Value Equality**: Records compare by value, not reference - perfect for DTOs
- **Non-Destructive Mutation**: `with` creates a copy with specified changes
...
- **Implicit Conversion**: No cast required - use for safe, lossless conversions
- **Explicit Conversion**: Requires cast - use when conversion might lose information
