...
## Section 1: Safety & Idempotency Fundamentals 🛡️
...
| **PATCH** | ⚠️ DEPENDS | `{"age": 30}` is idempotent. `{"age": "+1"}` is NOT |
...
```csharp
// ✅ Idempotent - calling multiple times with same ID has same effect
[HttpPut("orders/{id}")]
...
## Section 3: ETags & Content Negotiation 🏷️
...
├─ YES (e.g., "set status to X")
│  └─ Use PUT or PATCH
└─ NO (e.g., "create new order")
   ├─ Can client generate ID?
   │  └─ YES → Use PUT with client-generated ID
   └─ Is it critical (payments, orders)?
      └─ YES → MUST use Idempotency-Key header
```
