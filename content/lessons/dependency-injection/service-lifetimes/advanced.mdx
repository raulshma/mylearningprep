...
```csharp
// ⚠️ Scoped captured in Singleton
...
| Scoped    | ⚠️ Usually no           | Typically one thread per HTTP request  |
| Transient | ⚠️ Usually no           | New instance per injection             |
...
- 🧵 **Thread safety** - Required for singletons, usually not for scoped
- 🔑 **Keyed services** - .NET 8+ feature for multiple implementations
- 🗑️ **Disposal** - Container handles `IDisposable`/`IAsyncDisposable`
</InfoBox>
