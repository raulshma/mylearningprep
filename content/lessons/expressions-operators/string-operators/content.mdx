...
Strings in JavaScript are **immutable** — they can't be changed after creation. When you "modify" a string, you're actually creating a new string. This is why building long strings with `+=` in a loop can be slow.
</KeyConcept>
...
const badge = `${user.name} ${user.isAdmin ? '👑' : ''}`;
// "Alice 👑"
```
...
<InfoBox type="tip">
**Pro Tip**: Template literals are not just syntactic sugar — they're more readable, support multi-line strings naturally, and enable powerful patterns with tagged templates.
</InfoBox>
...
You've mastered string operators! Next up: **Conditional (Ternary) Operator** — the compact way to write inline conditionals.
</KeyConcept>
