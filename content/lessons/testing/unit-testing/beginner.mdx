...
```csharp
// ✅ EXCELLENT - Clear and descriptive
[Fact]
...
{
    private static ShoppingCart _cart = new ShoppingCart();  // ❌ Shared state!
...
Your tests should pass whether they run in order A→B→C, in reverse C→B→A, or in parallel. xUnit even runs tests in parallel by default!
</KeyConcept>
...
```comparison
{
  ...
  "left": {
    "title": "High Coverage ≠ Good Tests",
