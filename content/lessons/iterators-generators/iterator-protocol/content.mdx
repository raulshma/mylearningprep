...
Ever wondered how `for...of` loops know how to get the next item from an array? The answer is the **Iterator Protocol** — a standardized contract that defines how to produce a sequence of values, one at a time.
...
1. **Call `next()`** → Belt moves one position
2. **Get `value`** → Pick up the item at current position
3. **Check `done`** → Is the belt empty?
...
worker.next(); // Picks first 📦, belt has 2 left
worker.next(); // Picks second 📦, belt has 1 left
worker.next(); // Picks third 📦, belt is empty
...
Iterators are **forward-only** — you can't go back:
...
// ❌ Can't go back to 1!
// ❌ No iter.previous() method exists
...
Let's build an iterator from scratch — a countdown timer:
...
Here's a more practical example — iterating over a range of numbers:
...
Now that you understand the Iterator Protocol, you're ready to learn about the **Iterable Protocol** — which defines how objects advertise that they can be iterated with `for...of` loops!
</KeyConcept>
