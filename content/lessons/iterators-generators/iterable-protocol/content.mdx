...
You've learned how iterators work with `next()`. But how does JavaScript know an object *can* be iterated? That's where the **Iterable Protocol** comes in — it's the handshake that says "Hey, I can give you an iterator!"
...
Without a library card (Symbol.iterator), you can't borrow books (iterate). The card doesn't give you books directly — it gives you *access* to the borrowing system.
...
```javascript
// ❌ If we used a string property name
const obj = {
  iterator() { /* ... */ }  // Could conflict with user's "iterator" property
};

// ✅ Symbols are unique and won't conflict
const obj = {
...
// ⚠️ Don't spread infinite iterables!
// [...infiniteNumbers] // 💥 Infinite loop!

// ✅ Use with break or take a limited amount
...
You now understand both protocols! Next, learn about **Generator Functions** — a powerful syntax that makes creating iterators incredibly easy with `function*` and `yield`.
</KeyConcept>
