...
JavaScript is a "weakly typed" language — it automatically converts types when needed. This can be convenient, but it's also the source of many bugs. Let's demystify how JavaScript's automatic type coercion works!

---

## Section 1: The Invisible Converter

### The Helpful (But Sometimes Annoying) Assistant 🤖

Imagine you have an assistant who tries to be helpful by "fixing" things without asking:

- You say "add 5 and 3" → They give you 8 ✅
- You say "add '5' and 3" → They give you "53" 😱
...
"5" * "2"       // 10 (both → numbers, then multiply)
true + true     // 2 (both → numbers: 1 + 1)
...
The `+` operator has a split personality — it does both addition AND concatenation:
...
!"hello"        // false (truthy → true → false)
!0              // true (falsy → false → true)
...
### Strict Equality (===) — The Safe Choice
...
null > 0            // false (null → 0, but special handling)
...
Logical operators don't always return booleans — they return one of the operands:
...
### Nullish Coalescing (??) — The Modern Alternative
...
```javascript
// ❌ || can override valid falsy values
