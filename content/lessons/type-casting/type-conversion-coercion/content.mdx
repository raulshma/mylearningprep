...
|:---------|:-------------|:---------------|:--------|
| **Conversion** | You (the developer) | Using functions like `Number()`, `String()` | `Number("42")` → `42` |
| **Coercion** | JavaScript (automatically) | During operations | `"5" + 3` → `"53"` |

<InfoBox type="info">
**Key Insight**: Both conversion and coercion change a value from one type to another. The difference is *who* initiates the change — you or JavaScript.
</InfoBox>
...
let tax = 10;
let total = price + tax;
console.log(total); // "10010" 😱 Not what we wanted!

// With understanding:
let total = Number(price) + tax;
console.log(total); // 110 ✅ Perfect!
```
...
The `+` operator is special — it does both addition AND string concatenation:
