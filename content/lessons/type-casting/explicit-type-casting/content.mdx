...
When you explicitly convert types, you're telling JavaScript exactly what you want. No surprises, no guessing — just predictable, reliable code. Let's master every conversion function in your toolkit!
...
With explicit conversion, you're the translator — you control exactly how data transforms from one type to another.

```javascript
// The problem with implicit coercion
const userAge = "25";
const nextYear = userAge + 1;
console.log(nextYear); // "251" 😱 Oops!

// The explicit solution
const userAge = "25";
const nextYear = Number(userAge) + 1;
console.log(nextYear); // 26 ✅ Perfect!
```
...
| `parseInt()` | Integer | `parseInt("42px")` → `42` |
| `parseFloat()` | Float | `parseFloat("3.14em")` → `3.14` |
...
```javascript
// ❌ Bug: empty string passes as valid
if (Number(userInput) >= 0) { /* ... */ }

// ✅ Better: check for empty first
...
These functions are more forgiving — they extract numbers from the beginning of strings:
...
!"hello"      // false (truthy → boolean → negate)
...
// ❌ Don't use isNaN() - it coerces first!
...
// ✅ Or check the conversion result
const num = Number(input);
if (Number.isNaN(num)) {
...
|:----------|:---------|:--------|:-------|
| Any → Number | `Number()` | `Number("42")` | `42` |
| String → Integer | `parseInt()` | `parseInt("42px")` | `42` |
| String → Float | `parseFloat()` | `parseFloat("3.14em")` | `3.14` |
| Any → String | `String()` | `String(42)` | `"42"` |
| Number → String (base) | `.toString(radix)` | `(255).toString(16)` | `"ff"` |
| Any → Boolean | `Boolean()` | `Boolean("")` | `false` |
| Any → Boolean | `!!` | `!!"hello"` | `true` |

<KeyConcept title="What's Next?">
Now that you've mastered explicit conversion, let's explore the other side — **Implicit Type Casting**. Understanding when and how JavaScript automatically converts types will help you write bug-free code!
</KeyConcept>
