...
- Press **A1** → Dispense Cola
- Press **A2** → Dispense Sprite  
- Press **B1** → Dispense Water
- Press **anything else** → Show "Invalid selection"
...
Sometimes fall-through is useful — grouping cases that share code:
...
switch (day) {
  ...
  case "Saturday":
  case "Sunday":
    console.log("Weekend - time to relax! 🏖️");
    break;
  default:
    console.log("Invalid day");
}
// Output: "Weekend - time to relax! 🏖️"
```
...
// ✅ Use blocks for case-specific variables
switch (action) {
  case "create": {
...
// ✅ Good for if...else - ranges and complex conditions
...
You've mastered switch statements! Next up: the **ternary operator** — a concise way to write simple conditionals in a single line.
</KeyConcept>
