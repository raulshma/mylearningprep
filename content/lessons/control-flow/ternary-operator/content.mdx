...
The ternary operator is JavaScript's only operator that takes three operands. It's a compact way to write simple `if...else` statements in a single expression — perfect for inline conditionals and assignments.
...
You don't need a long explanation — just a quick answer based on one condition:
...
The ternary operator is an **expression** — it returns a value. This means you can use it inside other expressions, JSX, template literals, and anywhere a value is expected!
</KeyConcept>
...
const weather = temperature > 30 ? "🔥 Hot" : temperature > 20 ? "😎 Nice" : "🥶 Cold";
console.log(weather);
...
```javascript
// ✅ Formatted nested ternary (acceptable)
const accessLevel = 
  role === "admin"   ? "full" :
  role === "editor"  ? "write" :
  role === "viewer"  ? "read" :
...
// ✅ Alternative: each condition on its own line
const message = 
...
// ✅ Or extract to a function with clear logic
...
// ✅ Better - if...else is clearer for actions
if (isValid) {
  saveData();
} else {
  showError();
...
// ✅ Better - use if...else or extract logic
...
You've mastered the ternary operator! Next up: **Exception Handling** — learn to gracefully handle errors with try, catch, and finally.
</KeyConcept>
