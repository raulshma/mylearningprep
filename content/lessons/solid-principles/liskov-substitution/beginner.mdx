...
```
     ...
     ┌─────────────────────────────┐
     │   For each bird:            │
     │     bird.Fly()              │ ◀── Works for eagles, sparrows...
     │                             │
     └─────────────────────────────┘
                 │
                 │ But then...
     ...
     ┌─────────────────────────────┐
     │   💥 CRASH! 💥              │
     │   Penguin throws exception! │ ◀── Penguins can't fly!
     │                             │
     └─────────────────────────────┘
```
...
This is another famous example:

- A square IS a special rectangle (same width and height)
- But in code, they behave differently!

```
     Rectangle:               Square (Breaking LSP):
     ┌─────────────┐          ┌─────┐
     │ width = 5   │          │     │  If you set width = 5
     │ height = 3  │          │     │  height also becomes 5!
     │             │          │     │
     └─────────────┘          └─────┘
     
     Code: rect.Width = 5; rect.Height = 3;
     Expected area: 15
     Square gives: 9 ❌ SURPRISE!
```
...
<ProgressCheckpoint section="lsp-introduction" xpReward={25} />
