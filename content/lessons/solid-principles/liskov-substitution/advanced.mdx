# Liskov Substitution Principle (LSP) — Advanced
...
// IProducer<Animal> <- IProducer<Dog> ✅ (Dog is more specific)
...
// IConsumer<Dog> <- IConsumer<Animal> ✅ (Animal is more general)
...
public class OrderResponseV2 : OrderResponseV1
{
    ...
    // ❌ Changing Status to enum breaks string consumers
    // ❌ Making Total nullable breaks positive-assumption
    // ❌ Removing OrderId breaks deserialization
}
```
...
| Add new enum value | ⚠️ | Old clients may break on unknown value |
...
- ↕️ **Variance** enforces LSP at the type system level in C#
- 🌐 **API versioning** is architecture-level LSP
- 🧪 **Contract tests** catch violations across implementations
