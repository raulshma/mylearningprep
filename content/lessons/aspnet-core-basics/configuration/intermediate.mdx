...
|:---------|:-------|:---------|
| 1 (lowest) | appsettings.json | Default values |
| 2 | appsettings.{Env}.json | Environment overrides |
| 3 | User Secrets | Dev-only secrets |
| 4 | Environment Variables | Container/cloud config |
| 5 (highest) | Command Line | Runtime overrides |

<ProgressCheckpoint section="config-basics" xpReward={15} />
...
<ProgressCheckpoint section="config-intermediate" xpReward={15} />

---

## Section 3: The Options Pattern
...
- Use `IConfiguration` for direct access, Options pattern for strongly-typed access
- User Secrets keep sensitive data out of source control during development
- Environment variables use `__` instead of `:` for hierarchy
</InfoBox>

<ProgressCheckpoint section="config-advanced" xpReward={10} />
