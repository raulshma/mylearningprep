...
/**
 * POST /api/ai-tools/star-framework
 * Generate STAR framework response - Pro and Max only
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { situation, questionType } = body as {
      situation: string;
      questionType?: string;
    };
    ...
    const result = await generateSTARFramework(
      situation,
      questionType,
      apiKey ?? undefined,
    ...
    console.error("STAR framework generation error:", error);
    return NextResponse.json(
