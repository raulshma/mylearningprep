  ...
  return (
    <section className="py-24 md:py-32 px-6 bg-background">
      <div className="max-w-6xl mx-auto">
        ...
        >
          <p className="text-blue-600 dark:text-blue-400 font-medium mb-4">Pricing</p>
          ...
          {PRICING_TIERS.map((tier, index) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`relative h-full bg-gradient-to-b ${tierGradients[tier.id]} rounded-[2rem] p-8 flex flex-col overflow-hidden transition-all duration-500 ${
                    tier.featured 
                ...
                >
                  ...
                  {/* Icon */}
                  <div className={`w-12 h-12 rounded-xl bg-background/80 backdrop-blur flex items-center justify-center mb-5 ${tierAccents[tier.id]}`}>
                    <Icon className="w-6 h-6" />
                  </div>

                  {/* Tier name */}
                  <h3 className="text-xl font-semibold text-foreground mb-1">{tier.name}</h3>
                  <p className="text-sm text-muted-foreground mb-6">{tier.shortDescription}</p>
                  ...
                  {/* CTA */}
                  <Link href={tier.href || `/pricing?plan=${tier.id}`} className="w-full">
                    <Button
                      size="lg"
